...
library(ggplot2)
if (!is.null(guide$reverse) && length(guide$reverse) == 1 && !is.na(guide$reverse) && guide$reverse) {
...
fit %>% gg_tsresiduals()
report(fit)
fit %>% augment() %>%  features(.resid, ljung_box, lag = 8, dof = 4)
```
...
## Ejercicio: Selección de modelo para las ventas de H02

**Escoja distintos modelos ARIMA estacionales y, con base en la información arrojada, seleccione los 3 mejores modelos que encuentre.**
...
renderPrint({
  modelo2() %>% augment() %>% features(.resid, ljung_box, lag = 36, dof = nrow(tidy(modelo2()
               )
          )
                                      )
})
